...
#define CORE_LOCAL_COMPUTE_ZDROP() \
...
__global__ void gasal_local_kernel(uint32_t *packed_query_batch, uint32_t *packed_target_batch,  uint32_t *query_batch_lens, uint32_t *target_batch_lens, uint32_t *query_batch_offsets, uint32_t *target_batch_offsets, gasal_res_t *device_res, gasal_res_t *device_res_second, uint4 *packed_tb_matrices, int n_tasks, uint32_t max_query_len, short2 *global_inter_row)
{
	...
	int32_t* diag_maxHH_global = (int32_t*)(global_inter_col_p + max_query_len*(blockDim.x/8)*gridDim.x);
	int32_t* global_idx = (int32_t*)(global_inter_row + max_query_len*(blockDim.x/warp_len)*gridDim.x*4);

	int stretch = 8;
	bool active, zdropped;

	int32_t* diag_maxHH = (int32_t*)(shared_maxHH+(threadIdx.x/32)*packed_len*2*32);
	int32_t* shared_job = shared_maxHH+(blockDim.x/32)*packed_len*2*32+(tx/32)*28;
	...
	for (u = job_start_idx; u < job_end_idx; u++) {
		...
		i = global_idx[u];
		if (i== STREAM_BATCH_SIZE) continue;
		else if (i > STREAM_BATCH_SIZE) {
			...
			for (j = 0; j < job_per_query*2; j++) {
				if ((j*warp_len + warp_id) < max_query_len*2) {
					diag_maxHH_global[warp_num*max_query_len*2+j*warp_len + warp_id] = INT_MIN;	
				...
				*/
				...
				while (completed_band <= band_end) {
					...
					for (y = 0; y < stretch; y++) {
							...
							for (k = 28; k >= 0 && ridx < read_len; k -= 4) {
								...
								if (band_query == stretch_start || band_query == stretch_end) {
									...
									for (l = 28, m = 1; m < 9; l -= 4, m++) {
										CORE_LOCAL_COMPUTE_ZDROP();
									}
								} else {
									#pragma unroll 8
									for (l = 28, m = 1; m < 9; l -= 4, m++) {
										CORE_LOCAL_COMPUTE_ORIGINAL_ZDROP();
									}
								}
						...
						diag_idx = (band_query+band_target)<<3;
						for (m = 0; m < packed_len; m++) {
							prev_maxHH = __reduce_max_sync(same_threads, diag_maxHH[(((diag_idx+m)&15)<<5)+real_warp_id]);
							diag_maxHH[(((diag_idx+m)&15)<<5)+real_warp_id] = INT_MIN;
							if (warp_id==m) temp = prev_maxHH;
						}
						//maxHH = max(maxHH, (temp>>16));
												
						if (warp_id < packed_len) {
							diag_maxHH_global[warp_num*max_query_len*2+diag_idx + warp_id] = max(diag_maxHH_global[warp_num*max_query_len*2+diag_idx+ warp_id], temp);
						}
				...
				*/
				...
				for (diag_idx = j<<3; diag_idx < last_diag; diag_idx++) {
					if (diag_idx <prev_maxHH) {
						temp = diag_maxHH_global[warp_num*max_query_len*2+diag_idx];
						if ((temp>>16) > maxHH) {				
							maxHH = temp>>16;
							maxXY_y = (temp&65535);
				...
				*/
				...
				while (completed_band <= band_end) {
					...
					if (band_target <= band_end) {
						...
						for (y = 0; y < stretch; y++) {
							if (stretch_start <= band_query && band_query <=stretch_end) {
				...
				//same_threads = __match_any_sync(0xffffffff, warp_num);
				
				if (j >= total_diags) {
					prev_maxHH = __reduce_max_sync(same_threads, maxHH);
					if (1 == __popc(((0xffffffff) << (tx % 32))&__ballot_sync(same_threads, prev_maxHH==maxHH)) && prev_maxHH==maxHH) {
						/*
						shared_job[4+(warp_num&3)] = maxHH;
						shared_job[8+(warp_num&3)] = maxXY_x;
						shared_job[12+(warp_num&3)] = maxXY_y;
						*/
						device_res->aln_score[i] = maxHH;//copy the max score to the output array in the GPU mem
						device_res->query_batch_end[i] = maxXY_x;//copy the end position on query_batch sequence to the output array in the GPU mem
						device_res->target_batch_end[i] = maxXY_y;//copy the end position on target_batch sequence to the output array in the GPU mem
				...
				__syncwarp();
				//DEBUG =max(DEBUG, __popc(__match_any_sync(__activemask(), warp_num)));
			...
			if (warp_id==0) {
				device_res->aln_score[i] = shared_job[4+(warp_num&3)];//copy the max score to the output array in the GPU mem
				device_res->query_batch_end[i] = shared_job[8+(warp_num&3)];//copy the end position on query_batch sequence to the output array in the GPU mem
				device_res->target_batch_end[i] = shared_job[12+(warp_num&3)];//copy the end position on target_batch sequence to the output array in the GPU mem
			}
			*/
...
template <typename T, typename S, typename B>
__global__ void gasal_zdrop_pred(uint32_t *packed_query_batch, uint32_t *packed_target_batch,  uint32_t *query_batch_lens, uint32_t *target_batch_lens, uint32_t *query_batch_offsets, uint32_t *target_batch_offsets, int n_tasks, uint32_t max_query_len, short2 *global_inter_row)
{
    const uint32_t tid = (blockIdx.x * blockDim.x) + threadIdx.x;//thread ID
	//if (tid >= n_tasks) return;
	int32_t i,k;
	...
	uint32_t packed_target_batch_idx, packed_query_batch_idx, read_len, ref_len, query_batch_regs, target_batch_regs, min_regs;

	uint32_t* target; uint32_t* query; 
	uint32_t t, q;
	uint32_t match_count;
	unsigned const mask = __match_any_sync(0xffffffff, warp_num);
	uint32_t const select_0 = 0b11001100110011001100110011001100;
	uint32_t const select_1 = 0b10001000100010001000100010001000;
	uint32_t match;
	bool zdropped; 
	int zdrop = 400;
	int32_t* global_idx = (int32_t*)(global_inter_row + max_query_len*(blockDim.x/warp_len)*gridDim.x*4);
	...
	for (i = job_start_idx; i < job_end_idx; i++) {
		...
		target = packed_target_batch + packed_target_batch_idx;
		query = packed_query_batch + packed_query_batch_idx;
		
		min_regs = min(target_batch_regs, query_batch_regs);
		
		match_count = 0;

		for (k = 0; k < min_regs; k+=warp_len) {
			if (k+warp_id >= min_regs) break;
			t = target[k+warp_id];
			q = query[k+warp_id];
			/*
			for (int j = 28; j >= 0; j-=4) {
				if (((t >> j)&15) == ((q>>j)&15)) match_count++;
			}
			*/
			match = ~(t^q);	//bitwise XNOR 
			match = (match & (match <<2)) & select_0;
			match = (match & (match <<1)) & select_1;
			match_count += __popc(match);
			
		}

		__syncwarp(mask);

		match = __reduce_add_sync(mask, match_count);
		zdropped = (zdrop <= ((min_regs<<3) - match));
		
		if (warp_id==0) {
			
			global_idx[i] = (zdropped)? STREAM_BATCH_SIZE+i+1: i; //copy the max score to the output array in the GPU mem
			//device_res->query_batch_end[i] = match;//copy the end position on query_batch sequence to the output array in the GPU mem
			//device_res->target_batch_end[i] = (min_regs<<3) - match;//copy the end position on target_batch sequence to the output array in the GPU mem
		}
