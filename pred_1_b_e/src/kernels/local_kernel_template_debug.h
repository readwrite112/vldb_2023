...
__global__ void gasal_local_kernel(uint32_t *packed_query_batch, uint32_t *packed_target_batch,  uint32_t *query_batch_lens, uint32_t *target_batch_lens, uint32_t *query_batch_offsets, uint32_t *target_batch_offsets, gasal_res_t *device_res, gasal_res_t *device_res_second, uint4 *packed_tb_matrices, int n_tasks, uint32_t max_query_len, short2 *global_inter_row)
{
	...
	int32_t* diag_maxHH_global = (int32_t*)(global_inter_col_p + max_query_len*(blockDim.x/8)*gridDim.x+warp_num*max_query_len*2);

	int stretch = 8;

	int32_t* diag_maxHH = (int32_t*)(shared_maxHH+(threadIdx.x/32)*packed_len*2*32);
	int32_t diag_idx, temp, last_diag;
	...
	for (i = job_start_idx; i < job_end_idx; i++) {
		...
		for (j = 0; j < job_per_query; j++) {
			if ((j*warp_len + warp_id) < max_query_len) {
				global_inter_col_p[warp_num*max_query_len + j*warp_len + warp_id] = 0;	
			}
		}
		for (j = 0; j < job_per_query; j++) {
			if ((j*warp_len + warp_id) < max_query_len) {
				global_inter_col[warp_num*max_query_len + j*warp_len + warp_id] = initHD;	
			}
		}

		for (j = 0; j < job_per_query*2; j++) {
			if ((j*warp_len + warp_id) < max_query_len*2) {
				diag_maxHH_global[j*warp_len + warp_id] = 0;	
				//inter_row[warp_block_id*512 + j*warp_len + warp_id] = global_inter_row[warp_num*max_query_len + j*warp_len + warp_id];	
			}
		}
		
		for (m = 0; m < 16; m++) {
			diag_maxHH[real_warp_id + m*32] = 0;
		}
			...
			*/
			...
			while (completed_band <= band_end) {
				...
				for (y = 0; y < stretch; y++) {
					...
					for (m = 0; m < packed_len; m++) {
						...
						diag_maxHH[(((diag_idx+m)&15)<<5)+real_warp_id] = 0;
						if (warp_id==m) temp = prev_maxHH;
					}						
					if (warp_id < packed_len) {
						diag_maxHH_global[diag_idx + warp_id] = max(diag_maxHH_global[diag_idx+ warp_id], temp);
					}
			...
			for (diag_idx = j<<3; diag_idx < last_diag; diag_idx++) {
				if (diag_idx <prev_maxHH) {
					temp = diag_maxHH_global[diag_idx];
					if ((temp>>16) > maxHH) {				
						maxHH = temp>>16;
						maxXY_y = (temp&65535);
						...
						l = tl > ql? tl - ql : ql - tl;
						if (zdrop >= 0 && maxHH - (temp>>16) > zdrop + l*1) {
							/*
							zdropped = true;
							break;
							*/
